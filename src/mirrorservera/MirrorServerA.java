 ...
 * @author CHRISTIAN
 */
public class MirrorServerA {

    private final int SERVER_PORT = 9090;
    private final int CLIENT_PORT = 9091;  // Other server's port
    ...
    public void go(){
        ...
        this.clientThread = new Thread(new Runnable() {
            @Override
            public void run() {
                System.out.println("Thread A running successfully ");
                try {
                    ...
                    while (true) {
                        ...
                        System.out.println("Successfully sent Token: " + newToken+ " to Server B ");
                    }
                } catch (IOException exc) {
                    exc.printStackTrace();
                }
